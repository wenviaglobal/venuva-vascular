...
const Footer = () => {
  ...
  return (
    <footer className="bg-hospital-teal text-white pt-24 pb-12 overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 border-b border-white/10 pb-20">
          ...
          <div className="lg:col-span-3 space-y-8">
            <Link to="/" className="flex flex-col gap-1">
              <span className="text-2xl font-black tracking-tight text-white uppercase">{brand.name}</span>
              <span className="text-[10px] font-bold text-hospital-amber tracking-[0.3em] uppercase">
            ...
            <div className="flex gap-4">
              {[Facebook, Instagram, Twitter].map((Icon, i) => (
                <button key={i} className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white/50 hover:bg-hospital-amber hover:text-white transition-all">
                  <Icon size={18} />
                </button>
              ))}
            </div>
          ...
          <div className="lg:col-span-2">
            <h3 className="text-lg font-bold text-white mb-2 relative inline-block">
              Quick Links
              <span className="absolute -bottom-2 left-0 w-full h-1 bg-hospital-amber rounded-full"></span>
            </h3>
            <ul className="mt-10 space-y-4">
              {header.navLinks.map((link) => (
                <li key={link.name}>
                  <Link to={link.href} className="text-white/60 hover:text-white transition-colors font-bold text-sm block">
                    {link.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-4">
            ...
            <ul className="mt-10 space-y-6">
              ...
              <li className="flex items-start gap-4">
                <div className="mt-1 text-hospital-amber">
              ...
              {footer.contactUs.phones.map((num, i) => (
                <li key={i} className="flex items-center gap-4">
                  <div className="text-hospital-amber">
